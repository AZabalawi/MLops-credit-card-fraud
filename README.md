...
| Automated tests | [`tests/`](tests/) — 37 tests |
...
Both diagrams were drawn programmatically with matplotlib rather than by hand,
so the boxes and arrows match the code they describe. The rendered PNGs above
are the versioned artefacts; the one-off scripts that produced them are not kept
in the final tree, but they remain in this repository's Git history if a diagram
...
|---|---|---|
| `/` | GET | Browser demo page (see below) |
| `/api` | GET | Service description as JSON |
...
Opening the service root in a browser gives a small dashboard for showing the
model working live: it fills the form with a real transaction, posts it to
`/predict`, and displays the class, the probability against the decision
threshold, and the raw JSON the API returned. It is plain HTML, CSS and
JavaScript served from `app/static/`, with no build step and no extra
dependencies, and it calls the API over relative URLs so it behaves the same
locally and on Render. Nothing is computed in the browser - every number it
shows comes from a real `/predict` response.

The JSON service description that used to live at `/` is now at `/api`.
...
- every route, including `/`, `/api`, `/docs` and `/openapi.json`
- the demo page and its static assets, and that the two example transactions
  it loads are actually accepted and scored by the real model
...
members speaking and full speaker notes in the notes pane. Every figure on the
slides was read straight out of the project's own metrics and drift summary
files rather than typed by hand, so the numbers on screen match the numbers in
this repository. The deck was assembled by a one-off build script which, like
the diagram scripts, is kept in Git history rather than in the final tree.
...
| CI runs tests | Done | `pytest -v`, 37 tests |
